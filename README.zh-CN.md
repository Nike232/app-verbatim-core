...
命令会验证并规范化商店 URL，然后生成 `.github/workflows/app-verbatim.yml`：每日定时检查、手动触发、最小权限和去重回归 Issue 都已配置。已有文件不会被静默覆盖；需要固定不可变版本时可加 `--action-ref v0.5.5`。
...
Apple 连接器优先使用公开评论 RSS；当 RSS 返回空结果时，会回退到 Apple 公共商店页面展示的评论。当前回退最多提供 10 条可见评论，且不包含应用版本字段，因此版本回归检查会明确报告“证据不足”，不会猜测版本归因。公开商店行为可能变化，连接器仍需要持续维护。
