...
export function reportToMarkdown(report) {
  ...
  if (report.discoveredIssues?.length) {
    lines.push("", "## Discovered issues outside the built-in taxonomy", "");
    for (const issue of report.discoveredIssues) {
      lines.push(`### ${escapeMarkdown(issue.label)}`, "", `${issue.count} low-rating reviews · average ${issue.averageRating} stars · ${Math.round(issue.share * 100)}% of the full sample.`, "");
      for (const item of issue.evidence) lines.push(`- ${item.rating} stars · ${item.appVersion ? `v${escapeMarkdown(item.appVersion)} · ` : ""}${String(item.createdAt).slice(0, 10)} — “${escapeMarkdown(item.excerpt)}”`);
      lines.push("");
    }
  }
